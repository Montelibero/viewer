...
- Запуск через `docker compose`:
  ...
  **Важно:** Директория `site/` копируется в контейнер во время сборки. При изменении файлов в `site/` необходимо пересобрать контейнер, чтобы изменения вступили в силу:
  ```bash
